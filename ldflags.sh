...
function extra_ldflags() {
    ...
    local ldflags
    ldflags+=" -X xxxxx/pkg/version.gitVersion=${GIT_VERSION}"
    ldflags+=" -X xxxxx/pkg/version.gitCommit=${GIT_COMMIT_HASH}"
    ldflags+=" -X xxxxx/pkg/version.gitTreeState=${GIT_TREESTATE}"
    ldflags+=" -X xxxxx/pkg/version.buildDate=${BUILD_DATE}"
